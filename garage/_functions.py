...
def named_parameters_to_dict(model):
    dic = dict()
...
def obtain_evaluation_episodes(policy,
    ...
    try:
        policy_copy = deepcopy(policy).to()
    except:
        policy_copy = policy
    ...
    if policy_mask == None:
        ...
        print("Masking...")
        named_parameters = named_parameters_to_dict(policy_copy)
        for name, mask in policy_mask.items():
            named_parameters[name].requires_grad = False
            named_parameters[name] *= mask
...
def log_multitask_performance(itr, batch, discount, results, name_map=None, use_wandb=True):
    ...
    for seq_idx, task_name in enumerate(task_names):
        key_prefix = f"test/{seq_idx}/{task_name}/"
        if task_name in eps_by_name:
            ...
            if use_wandb:
                wandb.log({
                    key_prefix+'NumEpisodes': 0,
                    key_prefix+'AverageDiscountedReturn': np.nan,
                    key_prefix+'AverageReturn': np.nan,
                    key_prefix+'StdReturn': np.nan,
                    key_prefix+'MaxReturn': np.nan,
                    key_prefix+'MinReturn': np.nan,
                    key_prefix+'TerminationRate': np.nan,
                    key_prefix+'SuccessRate': np.nan,

                })

    return log_performance(itr, batch, discount=discount, results=results, prefix='Average', use_wandb=use_wandb)


def log_performance(itr, batch, discount, results, prefix='Evaluation',use_wandb = True):
    ...
    for eps in batch.split():
        ...
        elif 'is_success' in eps.env_infos:
            success.append(float(eps.env_infos['is_success'].any()))

    average_discounted_return = np.mean([rtn[0] for rtn in returns])
    if use_wandb:
        wandb.log({
            prefix+'NumEpisodes': len(returns),
            prefix+'AverageDiscountedReturn': average_discounted_return,
            prefix+'AverageReturn': np.mean(undiscounted_returns),
            prefix+'StdReturn': np.std(undiscounted_returns),
            prefix+'MaxReturn': np.max(undiscounted_returns),
            prefix+'MinReturn': np.min(undiscounted_returns),
            prefix+'TerminationRate': np.mean(termination),
        })

    if not prefix+'NumEpisodes' in results.keys():
        results[prefix+'NumEpisodes'] = []
        results[prefix+'AverageDiscountedReturn'] = []
        results[prefix+'AverageReturn'] = []
        results[prefix+'StdReturn'] = []
        results[prefix+'MaxReturn'] = []
        results[prefix+'MinReturn'] = []
        results[prefix+'TerminationRate'] = []
        results[prefix+'SuccessRate'] = []
        

    results[prefix+'NumEpisodes'].append(len(returns))
    results[prefix+'AverageDiscountedReturn'].append(average_discounted_return)
    results[prefix+'AverageReturn'].append(np.mean(undiscounted_returns))
    results[prefix+'StdReturn'].append(np.std(undiscounted_returns))
    results[prefix+'MaxReturn'].append(np.max(undiscounted_returns))
    results[prefix+'MinReturn'].append(np.min(undiscounted_returns))
    results[prefix+'TerminationRate'].append(np.mean(termination))

    
    
    if success:
        if use_wandb:
            wandb.log({prefix+'SuccessRate': np.mean(success)})
        results[prefix+'SuccessRate'].append(np.mean(success))
