...
class EpisodeBatch(TimeStepBatch):
    ...
    def __init__(self, env_spec, episode_infos, observations,
        ...
        if not _space_soft_contains(env_spec.observation_space,
                                    last_observations[0]):
            print('(garage/_dtypes.py/line 569) last_observations[0].shape:',last_observations[0].shape)
            print('(garage/_dtypes.py/line 570) env_spec.observation_space:',env_spec.observation_space)
...
def _space_soft_contains(space, element):
    ...
    if space.contains(element):
        ...
        if isinstance(element, dict):
            return space.flat_dim == np.prod(space.flatten(element).shape)
        else:
